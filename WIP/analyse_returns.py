...
def analyse_returns(returns, num_pairs_traded, num_pairs_chosen):
    ...
    print("Return on committed capital: " + str(return_on_committed_capital))
    print("Fully invested return: " + str(fully_invested_return))
    ...
    # Calmar ratio
    calmar_ratio = average / mdd if mdd > 0.0 else 0
...
def generate_risk_free_rate():
    date_cols = ['DATE']
    rates_data = pd.read_csv('../Backup/1yr_TreasuryYield(Daily).csv', parse_dates=date_cols)  # https://fred.stlouisfed.org/series/DGS1
    curr_year = rates_data.iloc[0]['DATE'].year
    rates = [rates_data.iloc[0]['Treasury Yield']]

    for i, row in rates_data.iterrows():
        if row['DATE'].year != curr_year or i == len(rates_data) - 1:
